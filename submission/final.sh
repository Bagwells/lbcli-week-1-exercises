...
# WRITE YOUR SOLUTION BELOW:
ADDR_INFO=$(bitcoin-cli -regtest -rpcwallet=btrustwallet getaddressinfo "$NEW_TAPROOT_ADDR" | jq -r '.desc')
check_cmd "Getting address info"

# STUDENT TASK: Extract the internal key (the x-only pubkey) from the descriptor
# WRITE YOUR SOLUTION BELOW:
INTERNAL_KEY=$(echo "$ADDR_INFO" | sed -n 's/^tr(\([^)]*\)).*/\1/p')
...
# WRITE YOUR SOLUTION BELOW:
TAPROOT_DESCRIPTOR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet getdescriptorinfo "tr($INTERNAL_KEY)" | jq -r '.descriptor')
...
# WRITE YOUR SOLUTION BELOW:
DERIVED_ADDR_RAW=$(bitcoin-cli -regtest -rpcwallet=btrustwallet deriveaddresses "$TAPROOT_DESCRIPTOR" | jq -r '.[]')
